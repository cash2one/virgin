        ...
        if auto.decodejwt(request.form['jwtstr']):
            ...
            # try:
                ...
                for i in item:
                    ...
                    #1待安置座位（0 2） 2待付款（1） 3待就餐（3） 4已就餐（4） 5已退单（6 7） 6失效订单（567）
                    if i['status'] in [0,2]:
                        data['time'] ='<font size=\"2\">'+'等待时间：'+'<font size=\"2\" color=\"red\">'+str(45-int((datetime.datetime.now()-i['add_time']).total_seconds()/60))+'分钟'+'</font></font>'
                        data['status'] = '待安置座位'
                    elif i['status'] ==1:
                        data['time'] ='<font size=\"2\">'+'剩余付款时间：'+'<font size=\"2\" color=\"red\">'+str(45-int((datetime.datetime.now()-i['add_time']).total_seconds()/60))+'分钟'+'</font></font>'
                        data['status'] = '待付款'
                    elif i['status'] ==3:
                    ...
                    for dis in i['dis_message']:
                        ...
                        if dis['dis_type'] == '1':
                            ...
                            pass
                    data['youhui'] = y_list
                    data['yingfu'] = str('%.2f' % (float(i['total']) - float(dis_amounts)))
                    data['yajin'] =  str('%.2f' % (float('%.2f' % (float(i['total']) - float(dis_amounts))) * 0.1))
                    data['dianfu'] = str('%.2f' % (float('%.2f' % (float(i['total']) - float(dis_amounts))) * 0.9))
                    restaurant = mongo.restaurant.find({"_id":ObjectId(i['restaurant_id'])})
                    for r in restaurant:
    ...
    if request.method=='POST':
        if auto.decodejwt(request.form['jwtstr']):
            ...
            try:
                ...
                item = mongo.order_groupinvite.find({"$or":[{"master_id":request.form['master_id']},{"friends":{"$in":[request.form['master_id']]}}]}).sort("start_time", pymongo.DESCENDING)[star:end]
                data = {}
                list = []
                ...
                for i in item:
                    ...
                    if i['status'] == 'wait_friends':
                        ...
                        json['time'] = '<font size=\"4\" color=\"#feb71d\">'+'距离活动结束还有 '+'</font><font size=\"5\" color=\"red\">'+str(45-int((datetime.datetime.now()-i['start_time']).total_seconds()/60))+'</font><font size=\"4\" color=\"#feb71d\">'+' 分钟'+'</font>'
                        json['status'] = '邀请好友进行时'
                        json['people_num'] ='<font size=\"4\">还有 '+'</font><font size=\"5\" color=\"red\">'+str(i['max_group'] - len(i['friends'])-1)+'</font><font size=\"4\">'+' 名好友待邀请</font>'
                        ...
                        json['time'] = '<font size=\"4\" color=\"#feb71d\">'+'已就餐'+'</font>'
                        json['status'] = '已就餐'
                        json['people_num'] ='<font size=\"4\">还有 '+'</font><font size=\"5\" color=\"red\">'+str(i['max_group'] - len(i['friends'])-1)+'</font><font size=\"4\">'+' 名好友待邀请</font>'
                    elif i['status'] == 'timeout':
                        json['time'] = '<font size=\"4\" color=\"#feb71d\">'+'活动已结束'+'</font>'
                        json['status'] = '已超时'
                        json['people_num'] ='<font size=\"4\">还有 '+'</font><font size=\"5\" color=\"red\">'+str(i['max_group'] - len(i['friends'])-1)+'</font><font size=\"4\">'+' 名好友待邀请</font>'
                    else:
                        json['time'] = ''
